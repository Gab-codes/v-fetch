...
describe("Basic Requests", () => {
  ...
  it("should perform PATCH request", async () => {
    const res = await client.patch<{ id: number; title: string }>(
      "/products/1",
  ...
  it("should perform PUT request", async () => {
    const res = await client.put<{ id: number; title: string }>("/products/1", {
  ...
  it("should perform DELETE request", async () => {
    const res = await client.delete<{ id: number; isDeleted: boolean }>(
      "/products/1",
    );
    expect(res.ok).toBe(true);
    if (res.ok) {
